...
class Meigee_ThemeOptions_Helper_Data extends Mage_Core_Helper_Abstract
{
 public function getThemeOptions ($themeOption) {
 	switch ($themeOption) {
		...
		break;
		case 'user-scalable':
			 if(Mage::getStoreConfig('meigee_general/layout/responsiveness') == 1){
		...
		break;
 		...
 		break;
 		case 'responsiveness':
 			return Mage::getStoreConfig('meigee_general/layout/responsiveness');
 		break;
 		case 'sidebar':
 		...
 		break; 		
 		case 'labels':
 			return Mage::getStoreConfig('meigee_general/productlabels');
 		break;
 ...
 public function getProductLabels ($product) {
 	$html = '';
 	if (Mage::getStoreConfig('meigee_general/productlabels/labelnew')):
		...
		if (isset($from) && $to->isLater($now)) $html .= '<span class="label-new"></span>';
	endif;
	if (Mage::getStoreConfig('meigee_general/productlabels/labelonsale') and $this->isOnSale($product)):
 ...
 public function prevnext ($product) {
 	if ($this->getThemeOptions('productpage_prevnext') == 'prevnext'):
		...
		*/
	    ...
	    if ($url <> $next_url): 
		$html = '<a class="product-prev" href="' . $next_url . '"><span><span>' . $this->__('Prev') . '</span></span></a>';
	    endif;
	    if ($url <> $prev_url):
	        $html .= '<a class="product-next" href="' . $prev_url . '"><span><span>' . $this->__('Next') . '</span></span></a>';
	    endif;
	...
	public function switchGrid() {
		if ((int)$this->getThemeOptions('responsiveness') !== 1):
			return 'css/grid_' . $this->getThemeOptions('responsiveness') . '.css';
		endif;
		return 'css/grid_responsive.css';
	}

	public function fancySwitcher(){
		if ((int)$this->getThemeOptions('fancybox') == 1 or (int)$this->getThemeOptions('ajax_general') == 1 or (int)$this->getThemeOptions('ajax_toolbar') == 1 or (int)$this->getThemeOptions('ajax_wishlistcompare') == 1):

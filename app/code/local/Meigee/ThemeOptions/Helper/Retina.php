 ...
 *
 */
class Meigee_ThemeOptions_Helper_Retina extends Mage_Core_Helper_Abstract
{
 
	public function getRetinaData ($data, $_item=0, $_itemparameter=0) {

 		if (Mage::getStoreConfig('meigee_general/appearance/retina')) {
			switch ($data) {
				case 'list':
					$html = 'data-srcX2="' . Mage::helper('catalog/image')->init($_item, 'small_image')->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE)->resize(580, null) . '"';
				break;
				case 'grid_large':
				...
				break;
				case 'upsell':
					$html = 'data-srcX2="' . Mage::helper('catalog/image')->init($_item, 'thumbnail')->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE)->resize(440, null) . '"';
				break;
				case 'crosssell':
				...
				break;
				case 'widget_slider':
					$html = 'data-srcX2="' . Mage::helper('catalog/image')->init($_item, 'small_image')->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE)->resize(550, null) . '"';
				break;
				case 'logo':
				...
				case 'wishlist_item_image':
					$html = 'data-srcX2="' . Mage::helper('catalog/image')->init($_item, 'small_image')->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE)->resize(420, null) . '"';
				break;
				case 'wishlist_sidebar_item':
					$html = 'data-srcX2="' . Mage::helper('catalog/image')->init($_item, 'thumbnail')->constrainOnly(TRUE)->keepAspectRatio(TRUE)->keepFrame(FALSE)->resize(116, null) . '"';
				break;
				case 'more_views':
